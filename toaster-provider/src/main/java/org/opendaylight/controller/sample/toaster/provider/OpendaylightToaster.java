...
public class OpendaylightToaster extends AbstractMXBean
    ...
    public void init() {
        ...
        dataTreeChangeListenerRegistration = dataBroker.registerDataTreeChangeListener(
                DataTreeIdentifier.create(CONFIGURATION, TOASTER_IID), this);   //用于对于配置信息（darkness）的修改
    ...
    public void close() {
        ...
        if (dataTreeChangeListenerRegistration != null) {   //关闭配置信息修改
    ...
    public void onDataTreeChanged(final Collection<DataTreeModification<Toaster>> changes) {
        for (DataTreeModification<Toaster> change: changes) {
            ...
            if (rootNode.getModificationType() == WRITE) {
                //  SUBTREE_MODIFIED, WRITE, DELETE三种Type
    ...
    private void checkStatusAndMakeToast(final MakeToastInput input,
        ...
        final ListenableFuture<? extends CommitInfo> commitFuture =
            Futures.transformAsync(readFuture, toasterData -> {
                ...
                if (toasterStatus == ToasterStatus.Up) {
                    ...
                    //keep check of the amount of bread
                    // by $￥
                    if (outOfBread()) {
        ...
        Futures.addCallback(commitFuture, new FutureCallback<CommitInfo>() {
            ...
            public void onSuccess(final CommitInfo result) {
                // OK to make toast
                // 生成一个MakeToastTask到线程执行 执行它的call方法
                currentMakeToastTask.set(executor.submit(new MakeToastTask(input, futureResult)));
            }

            @Override
            public void onFailure(final Throwable ex) {
                if (ex instanceof OptimisticLockFailedException) { //OptimisticLockFailedException乐观锁 是 下面TransactionCommitFailedException的子类
    ...
    private class MakeToastTask implements Callable<Void> {
        // 这个是checkStatusAndMakeToast中addCallBack里onSuccess执行的
